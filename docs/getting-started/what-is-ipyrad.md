...
ipyrad2 is a command-line toolkit for assembling and analyzing RAD-seq and related reduced-representation genomic datasets (e.g., RAD, ddRAD, GBS, 2bRAD, 3RAD).

In practical terms, it helps you move from reads or mapped BAMs to assembled loci, variant calls, exported sequence or SNP datasets, and downstream population-genetic analyses, all within one workflow.
...
The assembled outputs of ipyrad2 assembly includes a VCF as well as a HDF5 database file. The latter can serve as a substrate for downstream analyses implemented in ipyrad2, or to generate filtered and formatted output files for external tools by using the formatting/conversion tools: `wex`, `lex`, and `snpex`.
...
<!-- , but the full file model and the exact RAD/WGS boundary are described in [Files and Data Types](./files-and-data-types.md). -->
