...
class PCA(object):
    ...
    def __init__(
        ...
        if imap is None:
            # No imap implies no minmap
            self.imap = {}
            self.minmap = {}
        else:
            self.imap, self.minmap = self._parse_imap_minmap(imap, minmap)
    ...
    def _parse_imap_minmap(self, imap, minmap):
    ...
    def _impute_kmeans(self, topcov=0.9, niters=5, quiet=False):
        ...
        for it, kmeans_mincov in enumerate(iters):
            ...
            subdata = impdata[:, subsample_snps(se.snpsmap, self._seed())]
